...
export function adminBlockRoutes() {
  const r = new Hono<AppBindings>()
    ...
    .post("/entries", zValidator("json", createBlockEntry), async (c) => {
      const db = dbFromCtx(c);
      const me = c.get("user")!;
    ...
    .get("/protected-domains", async (c) => {
      const db = dbFromCtx(c);
      return c.json({ domains: await getProtectedDomains(db) });
    })

    .put("/protected-domains", zValidator("json", setProtectedDomains), async (c) => {
      const db = dbFromCtx(c);
