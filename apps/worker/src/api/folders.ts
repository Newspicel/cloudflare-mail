...
export function foldersRoutes() {
  const r = new Hono<AppBindings>()
    ...
    .post("/", zValidator("json", createFolder), async (c) => {
      const db = dbFromCtx(c);
      const user = c.get("user")!;
    ...
    .patch("/:id", zValidator("json", updateFolder), async (c) => {
      const db = dbFromCtx(c);
      const user = c.get("user")!;
      const body = c.req.valid("json");
    ...
    .get("/:id/threads", typedQuery<{ limit?: string; cursor?: string }>(), async (c) => {
      const db = dbFromCtx(c);
      const user = c.get("user")!;
      ...
      const ids = await accessibleMailboxIds(db, user.id);
      if (ids.length === 0)
        return c.json({ threads: [], nextCursor: null } satisfies ThreadListDto);
    ...
    .post("/:id/threads", zValidator("json", fileThreads), async (c) => {
      const db = dbFromCtx(c);
      const user = c.get("user")!;
      const id = c.req.param("id");
      await requireOwnFolder(db, user.id, id);
