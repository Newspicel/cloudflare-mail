...
const serviceKeyAuth: MiddlewareHandler<AppBindings> = async (c, next) => {
  ...
  const mb = await db.query.mailbox.findFirst({
    // A delete-pending mailbox reads as gone — its key stops working at once.
    where: and(
      eq(mailbox.serviceKeyHash, hash),
      eq(mailbox.type, "service"),
...
export function svcRoutes() {
  const r = new Hono<AppBindings>()
    ...
    .post("/send", zValidator("json", serviceSend), async (c) => {
      const db = dbFromCtx(c);
      const mb = c.get("serviceMailbox")!;
