...
10. **Spamhaus listings refuse, they don't file.** DQS (`mail/dnsbl.ts`) runs only when an admin has stored a query key. A ZEN hit on the connecting host, or a DBL/ZRD hit on a name the sending side asserts (envelope/header From, EHLO), SMTP-rejects with the listing named so the sender can delist. Everything softer — abused-legit DBL codes, anything found in the body — only scores, and no listing may score at or above `SPAM_AT` on its own. Lookups stay best-effort: a DNS failure or error code never blocks mail. Public DNSBL zones don't work here at all (a Worker resolves via DoH, i.e. a public resolver).
